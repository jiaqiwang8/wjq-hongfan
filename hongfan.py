...
def check_url(url):
    ...
    try:
        url_full = f"{url}/api/switch-value/list?sorts=%5B%7B%22Field%22:%221-user%22%7D%5D&conditions=%5B%5D&_ZQA_ID=4dc296c6c69905a7"
        r = requests.get(url=url_full, headers=headers, verify=False, timeout=5)
        if r.status_code == 400 and 'nvarchar' in r.text:
